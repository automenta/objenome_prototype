 ...
 * information is available directly from the tree. Use {@link #root()} to
...
public class STGPIndividual extends AbstractIndividual {
     ...
     * constructed from
     */
    public static final GP.GPKey<Node[]> SYNTAX = new GP.GPKey<>();

    /**
     * The key for setting and retrieving the required data-type for the root
     * node
     */
    public static final GP.GPKey<Class<?>> RETURN_TYPE = new GP.GPKey<>();

    /**
     * The key for setting and retrieving the maximum depth setting for program
     * trees
     */
    public static final GP.GPKey<Integer> MAXIMUM_DEPTH = new GP.GPKey<>();
...
//        }
     ...
     *         || index >= getLength())
     */
    public final void setNode(int index, Node node) {
        dataType = null;
        if (index > 0) {
            root.setNode(index, node);
        } else if (index == 0) {
            setRoot(node);
        } else {
    ...
    public int compareTo(Individual other) {
        Score x = score();
        Score y = other.score();
        return x==null ? (y == null /* both untested */ ? 0 : 1) : x.compareTo(y);
    }
}
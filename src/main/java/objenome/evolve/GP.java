...
import static objenome.op.Variable.EMPTY_VARIABLE_ARRAY;
 ...
 * SINGLETON scope
 */
public class GP<I extends Individual> extends Container {

    /**
     * The key for setting and retrieving the list of components.
     */
    protected static final GPKey<ArrayList<PopulationProcess>> COMPONENTS = new GPKey<>();
    ...
    public Variable[] getVariables() {
        ...
        return variables==null ? EMPTY_VARIABLE_ARRAY : variables.toArray(EMPTY_VARIABLE_ARRAY);
    }
...
//    }
    ...
    public <E extends Event> void resetStats() {
        for (Class<?> type : new ArrayList<>(stat.keySet())) {
            remove(type);
        }
     ...
     */
    public <T> GP set(GPKey<T> key, T value) {
    ...
    static void setContainerAware(GP config, Object value) {
     ...
     * convenience method
     */
    public <T> GP with(GPKey<T> key, T value) {

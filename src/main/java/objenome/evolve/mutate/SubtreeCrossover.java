...
public class SubtreeCrossover extends AbstractOperator implements Listener<ConfigEvent> {
     ...
     */
    public static final GP.GPKey<Double> TERMINAL_PROBABILITY = new GP.GPKey<>();
    ...
    public void onEvent(ConfigEvent event) {
        if (event.isKindOf(RandomSequence.RANDOM_SEQUENCE, TERMINAL_PROBABILITY, PROBABILITY, STGPIndividual.MAXIMUM_DEPTH)) {
    ...
    public STGPIndividual[] perform(OperatorEvent.EndOperator event, Individual... parents) {
        ...
        nodesOfType(program2.root(), subtree1.dataType(), 0, matchingNodes, matchingIndexes);
    ...
    protected int crossoverPoint(STGPIndividual individual) {
        ...
        int noTerminals = individual.root().terminalCount();
        ...
        if ((noNonTerminals > 0) && (random.nextDouble() >= terminalProbability)) {
            ...
            return individual.root().nthNonTerminalIndex(f);
        } else {
            int t = random.nextInt(noTerminals);
            return individual.root().nthTerminalIndex(t);
        }
    }

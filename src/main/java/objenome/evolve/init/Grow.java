...
public class Grow implements STGPInitialisation, Listener<ConfigEvent> {
     ...
     * </ul>
     */
    protected void setup(GP config) {
    ...
    public void onEvent(ConfigEvent event) {
        if (event.isKindOf(STGP.PROBLEM, RANDOM_SEQUENCE, SIZE, SYNTAX, RETURN_TYPE, MAXIMUM_INITIAL_DEPTH, MAXIMUM_DEPTH, ALLOW_DUPLICATES)) {
        ...
        if (event.isKindOf(STGP.PROBLEM, RETURN_TYPE)) {
            dataTypesTable = null;
        }
        if (event.isKindOf(STGP.PROBLEM, SYNTAX)) {
            updateSyntax();
        }
    }
     ...
     */
    @Override
    public Population createPopulation(Population survivors, GP config) {

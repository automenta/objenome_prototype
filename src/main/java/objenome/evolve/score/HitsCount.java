...
public class HitsCount extends STGPScoreFunction implements Listener<ConfigEvent> {
     ...
     * evaluated
     */
    public static final GP.GPKey<Double[]> EXPECTED_OUTPUTS = new GP.GPKey<>();

    /**
     * The key for setting the acceptable error for each point to count as a hit
     */
    public static final GP.GPKey<Double> POINT_ERROR = new GP.GPKey<>();
    ...
    public DoubleScore.Minimize evaluate(Population population, Individual individual) {
        ...
        return new DoubleScore.Minimize(noWrong);
    }

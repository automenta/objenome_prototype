 ...
 * @since 2.0
 */
public class GenerationAverageNodesPerDepth extends AbstractStat<GenerationEvent.EndGeneration> {
        ...
        for (int d = 0; d < maxDepth; d++) {
            ...
            for (Individual individual : population) {
                if (individual instanceof STGPIndividual) {
                    noNodes += ((STGPIndividual) individual).root().nodesAtDepth(d).size();
                }
            }
